...
//	Path_SEL_RF = ODM_GetRFReg(pDM_Odm, ODM_RF_PATH_A, 0xb0, 0xfffff);
	...
	//set GNT_BT=0
	RegTempValue = ODM_GetBBReg(pDM_Odm, 0x764, bMaskDWord);
	RegTempValue |= 0x0800; /* bit11 = 1 */
	RegTempValue &= 0xefff; /* bit12 = 0 */
	ODM_SetBBReg(pDM_Odm, 0x764, bMaskDWord, RegTempValue);
...
//	ODM_SetRFReg(pDM_Odm, ODM_RF_PATH_A, 0xb0, 0xfffff, Path_SEL_RF);
	...
	RegTempValue = ODM_GetBBReg(pDM_Odm, 0x764, bMaskDWord);
	RegTempValue &= 0xe7ff; /* bit12 = bit11 = 0 */
	ODM_SetBBReg(pDM_Odm, 0x764, bMaskDWord, RegTempValue);
