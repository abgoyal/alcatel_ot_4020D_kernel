			...
			switch (poidparam->subcode){
				...
				case WOWLAN_DISABLE:
					...
					// 5. Download reserved pages and report media status if needed.
				 	if ((adapter_to_pwrctl(padapter)->wowlan_wake_reason != FWDecisionDisconnect) &&
						(adapter_to_pwrctl(padapter)->wowlan_wake_reason != Rx_DisAssoc) &&
						(adapter_to_pwrctl(padapter)->wowlan_wake_reason != Rx_DeAuth))
					{
						rtl8723a_set_FwJoinBssReport_cmd(padapter, RT_MEDIA_CONNECT);
						rtw_hal_update_ra_mask(padapter, 0, DM_RATR_STA_LOW);
					}
				 	if (adapter_to_pwrctl(padapter)->wowlan_wake_reason == Rx_GTK)
						DBG_871X("Recv GTK!\n");
					break;
				default:
					break;
...
void UpdateHalRAMask8192CUsb(PADAPTER padapter, u32 mac_id, u8 rssi_level)
{
	...
	if(pHalData->fw_ractrl == _TRUE)
	{
		...
		DBG_871X("update raid entry, mask=0x%x, arg=0x%x macid:%d raid:%d\n", mask, arg, mac_id, raid);

		rtl8192c_set_raid_cmd(padapter, mask, arg);

#ifdef CONFIG_BT_COEXIST
		if (check_fwstate(&padapter->mlmepriv, WIFI_STATION_STATE) && mac_id == 0) {
			u32 coex_mac_id = BT_COEX_RA_ID;
			u8 coex_raid = BT_COEX_RA_ID;
			u8 coex_arg = 0;
			u32 fallback_mask = mask & (~0xf0000000); //remove normal raid;
			u32 coex_mask = mask & (~0xf0000000); //remove normal raid
			u32 ratefilter= RATE_1M | RATE_2M | RATE_5_5M; //disable 1M/2M/5.5M when sco
			static u32 fallback_table[]=
			{
				0xf8ff015,	//BGN 40M 2ss
				0xff015,	//BGN 40M 1ss
			...
			coex_arg = coex_mac_id & 0x1f;//MACID
			coex_arg |= BIT(7);
			if (shortGIrate==_TRUE)
				coex_arg |= BIT(5);

			coex_mask |= ((coex_raid << 28) & 0xf0000000); //add coex_raid

			coex_mask &= (~ratefilter);

			/* RateFallBackMask for RetryRate */
			//fallback_mask = fallback_table[raid&0xF] & (~ratefilter);
			fallback_mask = fallback_mask & (~ratefilter);
			if(rtw_read32(padapter, REG_ARFR0) != fallback_mask)
				rtw_write32(padapter, REG_ARFR0, fallback_mask);

			DBG_871X("update coex raid entry, mask=0x%x, arg=0x%x fallback_mask:%x\n",
					coex_mask, coex_arg, fallback_mask);

			/* RaMask for InitRate */
			rtl8192c_set_raid_cmd(padapter, coex_mask, coex_arg);
		}
#endif
	}
	else
