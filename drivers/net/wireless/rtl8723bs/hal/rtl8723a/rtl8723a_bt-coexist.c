...
#define BT_COEX_VERSION	"v3.1@20130515"
...
//#define CONFIG_SCO_ANT_AT_BT
#ifdef CONFIG_SCO_ANT_AT_BT
	...
	)
{
	...
	switch (StateCmd)
	{
		...
		case STATE_CMD_MAC_CONNECT_COMPLETE:
		{
			...
			if (pBTInfo->BtAsocEntry[EntryNum].AMPRole == AMP_BTAP_JOINER)
			{
				RTPRINT(FIOCTL, IOCTL_STATE, ("StateConnecting\n"));
...
void btdm_1AntSetPSMode(PADAPTER padapter, u8 enable, u8 smartps, u8 mode)
{
	...
	if (enable == _TRUE) {
		if (rtw_check_iot_peer_ps_should_pspoll(padapter))
			smart_ps = 0;
		rtw_set_ps_mode(padapter, PS_MODE_MIN, smart_ps, mode);
...
static void btdm_1AntSetCoexTable(PADAPTER padapter, u32 reg_6c0_val, u32 reg_6c4_val)
{
	rtw_write32(padapter, 0x6c0, reg_6c0_val);
	rtw_write32(padapter, 0x6c4, reg_6c4_val);
}
...
void btdm_1AntSetPSTDMA_SCO_CMCC(PADAPTER padapter)
{
	...
	btdm_1AntSetCoexTable(padapter, 0x5a5a5a5a, 0x5a5a5a5a);
}
#endif
		...
		switch (type)
		{
...
#endif
						BTDM_SetFw3a(padapter, 0x53, 0x2c, 0x03, 0x10, 0x50);
				}
				break;
			case 2:	// A2DP Level-2
				if (btdm_Is1AntPsTdmaStateChange(padapter))
...
#endif
						BTDM_SetFw3a(padapter, 0x53, 0x25, 0x03, 0x10, 0x50);
				}
				break;
			case 3:	// BT FTP/OPP
				if (btdm_Is1AntPsTdmaStateChange(padapter))
...
#endif
						BTDM_SetFw3a(padapter, 0xd3, 0x30, 0x03, 0x10, 0x50);
			...
			case 5:	// for WiFi connected-busy & BT is Non-Connected-Idle
				if (btdm_Is1AntPsTdmaStateChange(padapter))
				{
					...
					BTDM_SetFw3a(padapter, 0x69, 0x15, 0x03, 0x31, 0x00);
				}
				break;
...
#endif
						BTDM_SetFw3a(padapter, 0x53, 0x1e, 0x03, 0x10, 0x50); //narrow duration for WiFi
				}
				break;
...
#endif
						BTDM_SetFw3a(padapter, 0x53, 0x12, 0x03, 0x10, 0x50);
				}
				break;
			...
			case 29: // WiFi DHCP/Site Survey & BT ACL busy
				if (btdm_Is1AntPsTdmaStateChange(padapter))
				{
					BTDM_SetFw3a(padapter, 0xa3, 0x1a, 0x1a, 0x1, 0x10);
					btdm_1AntSetCoexTable(padapter, 0x5afa5afa, 0x5afa5afa);
				}
				break;
			case 30: // WiFi idle & BT Inquiry
				if (btdm_Is1AntPsTdmaStateChange(padapter))
				{
					BTDM_SetFw3a(padapter, 0x93, 0x0a, 0x03, 0x14, 0x00);
				}
				break;
			case 31:  // BT HID
				if (btdm_Is1AntPsTdmaStateChange(padapter))
			...
			case 36: //SoftAP PowerSaving
				...
				break;
			case 37:  // BT A2DP/FTP/A2DP+HID/FTP+A2DP & WiFi LPS
				if (btdm_Is1AntPsTdmaStateChange(padapter))
				{
...
void btdm_1AntPtaParaReload(PADAPTER padapter)
{
	...
	rtw_write32(padapter, 0x6c8, 0xffffff);		// wifi break table
...
//	RTPRINT(FBT, BT_TRACE, ("[BTCoex], 1AntCoexProcessForWifiConnect!!\n"));
	...
	{
		if (adapter_to_pwrctl(padapter)->WiFiOnlyLowTrafficTdma) {
	...
	else
	{
		...
		switch (BtState)
		{
			...
			case BT_INFO_STATE_ACL_ONLY_BUSY:
				...
				else if (pBtCoex->c2hBtProfile == (BT_INFO_A2DP|BT_INFO_FTP|BT_INFO_HID))
				{
					RTPRINT(FBT, BT_TRACE, ("[BTCoex], BT PROFILE is A2DP_FTP_HID\n"));
					btdm_1AntSetPSTDMA(padapter, _TRUE, 0, _TRUE, 37);
				}
				else
				{
					if (pBtCoex->c2hBtProfile == BT_INFO_A2DP)
					{
						...
						if ((pBtCoex->AclTp > 0x3000) || (pBtCoex->lowPriorityTx > 1100))
						{
							RTPRINT(FBT, BT_TRACE, ("[BTCoex], BT PROFILE is A2DP change to A2DP+OPP\n"));
							RTPRINT(FBT, BT_TRACE, ("[BTCoex], AclTp=0x%x lowPriorityTx=%d\n",
								pBtCoex->AclTp, pBtCoex->lowPriorityTx));
							btdm_1AntSetPSTDMA(padapter, _TRUE, 0, _TRUE, 11);
							break;
						}
					}
...
void BTDM_1AntParaInit(PADAPTER padapter)
{
	...
	/* set bt acl busy state for BT busy wifi close/open issue*/
	if (!wifi_open) {
		RTPRINT(FBT, BT_TRACE, ("\n[BTCoex], wifi_open set default BT state\n"));
...
void BTDM_1AntForLowWiFiTraffic(PADAPTER padapter)
{
	...
	/* now TDMA all use RFON mode, and */
	/* LowTraffic have no power save use now, */
	/* so close it */
	return;
	...
	if (adapter_to_pwrctl(padapter)->pwr_mode != PS_MODE_ACTIVE && !pBtdm8723->bCurPsTdmaOn) {
...
void BTDM_SoftAP_PowerSave(PADAPTER padapter, u8 type)
{
	...
	/* set busy_traffic when softap connected & busy */
	//if ((pstapriv->asoc_sta_count > 2) && padapter->mlmepriv.LinkDetectInfo.bBusyTraffic)
	/* set busy_traffic when softap connected */
...
void BTDM_1AntWifiScanNotify(PADAPTER padapter, u8 scanType)
{
	...
	if (scanType)
	{
		...
		padapter->mlmepriv.scan_compensation = 0;
		if ((BtState == BT_INFO_STATE_SCO_ONLY_BUSY) ||
			(BtState == BT_INFO_STATE_ACL_SCO_BUSY))
			padapter->mlmepriv.scan_compensation = 15;
	}
...
void BTDM_1AntFwC2hBtInfo8723A(PADAPTER padapter)
{
	...
	if (pBtMgnt->ExtConfig.bBTBusy) {
		BTDM_RejectApAggregatedPacket(padapter, 1);
	} else {
		BTDM_RejectApAggregatedPacket(padapter, 0);
	}
...
void BTDM_1AntBtCoexist8723A(PADAPTER padapter)
{
	...
	if(pmlmepriv->android_btcex_bdhcp == _TRUE) {
	...
	)
{
	...
	if (BTDM_1Ant8723A(padapter) == _TRUE && (byte1 & BIT0)) //tdma on
	{
		if ((check_fwstate(&padapter->mlmepriv, WIFI_STATION_STATE) == _FALSE) &&
			(get_fwstate(&padapter->mlmepriv) != WIFI_NULL_STATE)) // for softap mode
		{
			...
			if (rtw_check_iot_peer_ps_should_pspoll(padapter)) {
				if(!(byte1 & BIT5)) { //NoPS = 0 psmode open
					RTPRINT(FBT, BT_TRACE, ("[BTCoex], change pspoll mode\n"));
...
//	PRT_POWER_SAVE_CONTROL	pPSC = GET_POWER_SAVE_CONTROL(pMgntInfo);
	...
	/* ips rf_on == type not call this function in old bt coex */
	if (rf_on == type)
		return;
...
{
	/* compat 8723bs New Bt Coex, do nothing */
}
		...
		)
{
	...
	pBTInfo = GET_BT_INFO(Adapter);
	pBtMgnt = &pBTInfo->BtMgnt;

	pBtMgnt->bManualControl = Flag;
}
...
{
	DBG_871X_LEVEL(_drv_always_, "BT Coexist version: " BT_COEX_VERSION "\n");
