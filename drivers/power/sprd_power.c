...
struct work_struct power_off_struct;	
struct mutex power_off_mutex;	

static void power_off_work(struct work_struct *work)	
{	
	int flag = 0;	
       //printk("Coming ....power_off_work\n");	
       mutex_lock(&power_off_mutex);	
       flag = power_off_flag;	
       mutex_unlock(&power_off_mutex);	
				
       if(flag == 1){	
               kernel_power_off();	
       }	
       usleep_range(2000,3000);	
       schedule_work(&power_off_struct);	
}
...
static int sprd_battery_set_property(struct power_supply *psy,	
								           enum power_supply_property psp,	
									    union power_supply_propval *val)	
{	
	struct sprd_battery_data *data = container_of(psy,	
				   ...
				   const char *buf, size_t count)
{
	...
	case SAVE_CAPACITY:
	{
		int temp = set_value - battery_data->poweron_capacity;   //in probe to read %
		printk("battery temp : %d\n", temp);
		if(abs(temp) > SPRBAT_VALID_CAP || 0 == set_value)
		{
			printk("battery power on capacity =%d , old value = %d\n", battery_data->poweron_capacity,set_value);
		}else{
			printk("battery power in old capacity = %d power on value = %d\n",set_value,battery_data->poweron_capacity);
			battery_data->capacity = set_value;
		}
...
void enable_usb_charge(struct sprd_battery_data *battery_data)
{
	...
	pluse_charging = 0;
	sprd_set_chg_cur(battery_data->cur_type);
...
static void charge_handler(struct sprd_battery_data *battery_data, int in_sleep)
{
	...
	if (!battery_data->charging && !battery_data->in_precharge &&
      ...
      /*	
      * test temp: 	
      *      echo 700 > /sys/class/power_supply/battery/temp	
      * set temp=700 to shutdown phone.	
      *	
      * temp > 550 stop charging	
      * temp > 600 shutdonw	
      * temp down to 520 , recharging	
      */	
      
       if (battery_data->temp > OTP_OVER_HIGH ||battery_data->temp  < OTP_OVER_LOW) 
	{	
       	battery_data->over_temp_flag = 1;	
       	battery_notify = 1;	
       	printk(KERN_ERR "battery temperature out temp,stop charging:%d\n", battery_data->temp);	
       	charge_stop(battery_data);	
		printk("stop charging ... \n");
       	if(battery_data->temp >OTP_OVER_HIGH_POW_OFF)	
       	{	
			printk(KERN_INFO "battery temperature out temp,shutdown:%d\n", battery_data->temp);	
	...
	if (!battery_data->charging && !battery_data->in_precharge &&
		...
		if (battery_data->temp > OTP_RESUME_LOW && battery_data->temp < OTP_RESUME_HIGH) {
			printk(KERN_ERR
			...
			if (ac_online){
				enable_ac_charge(battery_data);
				printk("ac charge enable...\n");
			}
			else{
				enable_usb_charge(battery_data);
				printk("usb charge enable...\n");
			}
		}
	}
       #endif	
	...
	if (temp > OTP_OVER_HIGH || temp < OTP_OVER_LOW) {
		...
		charge_stop(battery_data);
		printk("stop charge...\n");
		if (temp >OTP_OVER_HIGH_POW_OFF ||temp < OTP_LOW_POW_OFF) {
		    printk(KERN_INFO "battery temperature out temp,shutdown:%d\n", temp);
		    mutex_lock(&power_off_mutex);
		    if(power_off_flag != 1){
			power_off_flag = 1;
	...
	} else if (temp > OTP_CURRENT_LOW && temp < OTP_CURRENT_HIGH ) {
            if (battery_data->charging && !battery_data->over_voltage_flag) {
	        if (usb_online && (battery_data->cur_type != SPRD_USB_CHG_CUR_CHARG_DOWN)) {
                    printk("usb online, change current to 400ma");
	            battery_data->cur_type = SPRD_USB_CHG_CUR_CHARG_DOWN;
		    charge_stop(battery_data);
	            enable_usb_charge(battery_data);
	        }
	        else if (ac_online && (battery_data->cur_type != SPRD_AC_CHG_CUR_CHARG_DOWN)) {
                    printk("ac online, change current to 400ma");
		    battery_data->cur_type = SPRD_AC_CHG_CUR_CHARG_DOWN;
		    charge_stop(battery_data);
	            enable_ac_charge(battery_data);
	        }
            }
            ...
            if (battery_data->charging && !battery_data->over_voltage_flag) {
	        if (usb_online && (battery_data->cur_type != SPRD_USB_CHG_CUR)) {
                    printk("usb online, change current to 500ma");
	            battery_data->cur_type = SPRD_USB_CHG_CUR;
	            charge_stop(battery_data);
	            enable_usb_charge(battery_data);
	        }
	        else if (ac_online && (battery_data->cur_type != SPRD_AC_CHG_CUR)) {
                    printk("ac online, change current to 500ma");
		    battery_data->cur_type = SPRD_AC_CHG_CUR;
		    charge_stop(battery_data);
	            enable_ac_charge(battery_data);
	        }
...
static int sprd_battery_probe(struct platform_device *pdev)
{
	...
	data->battery.set_property = sprd_battery_set_property;
	...
	data->battery_timer.data = (unsigned long)data;
       ...
       INIT_WORK(&power_off_struct,power_off_work);
       mutex_init(&power_off_mutex);
...
retry_adc:
	...
	battery_data->poweron_capacity = data->capacity;
