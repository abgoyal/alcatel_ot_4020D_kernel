...
#define  CONFIG_TOUCHSCREEN_MSG2133A_PSENSOR

#ifdef CONFIG_TOUCHSCREEN_MSG2133A_PSENSOR
#include <linux/wakelock.h>
#endif
#define CONFIG_MSG2133A_ENABLE_AUTO_UPDATA
...
#endif
#ifdef CONFIG_HAS_EARLYSUSPEND//(for PR753200)
...
static const unsigned char MSG2138A_NEW_ITO_update_bin[]=
{
	#include "MSG2138A_update_V0x001.0x002_20140722.i"
};
static unsigned short curr_ic_major=0;
static unsigned short curr_ic_minor=0;
static int getFWPrivateVersion(void);
...
static ssize_t firmware_update_with_i ( struct device *dev,
                                       struct device_attribute *attr, const char *buf, size_t size )
{
   // u8 i;
//	struct msg2133_ts_data *msg2133_ts = dev_get_drvdata(dev);
...
static int Msg2133_GetInISize(char * config_name)
{
...
static int Msg2133_ReadInIData(char *config_name,
			      char *config_buf)
{
	struct file *pfile = NULL;
	struct inode *inode;
	...
	set_fs(KERNEL_DS);
	pos = 0;
	size=vfs_read(pfile, config_buf, fsize, &pos);
...
//	disable_irq(msg2133_ts->pdata->irq);
	...
	memcpy(fwname,buf,size);
	fwname[size- 1] = '\0';

	 inisize = Msg2133_GetInISize(fwname);

	printk("inisize = %d ,%s,%d\n ", inisize,fwname,size);
	if (inisize <= 0) {
	...
	if (Msg2133_ReadInIData(fwname, filedata)) {
		pr_err("%s() - ERROR: request_firmware failed\n",
					__func__);
		kfree(filedata);
		return -EIO;
	} else {
		printk("ft5x0x_ReadInIData successful\n");
	}
	//FwDataCnt=0;
	//memcpy(temp[FwDataCnt], filedata, 1024*94);
    	for (i = 0; i < 33; i++)
			{
				for (j = 0; j < 1024; j++)
				{
					temp[i][j] = filedata[i*1024+j];
				}
			}
                                     ...
                                     struct device_attribute *attr, char *buf)
{
	...
	tx_data[1] = 0x00;
	tx_data[2] = 0x2A;
	ret=HalTscrCDevWriteI2CSeq(FW_ADDR_MSG21XX_TP, &tx_data[0], 3);
	if(ret < 0)
	{
		return ret;
	}
	...
	version_major = MSG2138A_NEW_ITO_update_bin[0x7f4f]<<8|MSG2138A_NEW_ITO_update_bin[0x7f4e];
	version_minor = MSG2138A_NEW_ITO_update_bin[0x7f51]<<8|MSG2138A_NEW_ITO_update_bin[0x7f50];

    TP_DEBUG("*** firmware_version_show fw_version = %s*** %d %d=%d %d\n", fw_version,version_major,version_minor,curr_ic_major,curr_ic_minor);
	if(Major==1)
    return sprintf(buf, "msg2138A:data=%d%d,register=%d%d\n", version_major,version_minor,curr_ic_major,curr_ic_minor);
	else
	return sprintf(buf, "msg2133A:this tp didn`t used.register=%d%d\n", curr_ic_major,curr_ic_minor);
}
...
{
	#include "MSG2133A_update_0x000b_20140326.i"
};
...
static int GetDeviceIcType(void)
{
   // u8 i;
//	struct msg2133_ts_data *msg2133_ts = dev_get_drvdata(dev);
    u8 dbbus_tx_data[4];
    ...
    if ( dbbus_rx_data[0] == 2 )
    {
      ...
      //  TP_DEBUG ( "dbbus_rx version[0]=0x%x", dbbus_rx_data[0] );
       // printk ( "dbbus_rx version[0]=0x%x", dbbus_rx_data[0] );
        ...
        if ( dbbus_rx_data[0] == 3 ){
        printk ( "get ic type success==c32=\n" );
            return 0;
		}
        else{
        printk ( "get ic type fail-c33-\n" );
...
static void msg2133a_auto_update(void)
{
	...
	update_new_bin_major = MSG2138A_NEW_ITO_update_bin[0x7f4f]<<8|MSG2138A_NEW_ITO_update_bin[0x7f4e];
	update_new_bin_minor = MSG2138A_NEW_ITO_update_bin[0x7f51]<<8|MSG2138A_NEW_ITO_update_bin[0x7f50];
	...
	if(curr_ic_major == 790)
	{
		if(update_old_bin_minor> curr_ic_minor)
		{
			...
			firmware_update_with_i(NULL, NULL, NULL, 0);
		}
	}
	else if(1==curr_ic_major ||0==curr_ic_major)
	{
	 	if(update_new_bin_minor> curr_ic_minor)
		{
			...
			for (i = 0; i < 33; i++)
			{
				for (j = 0; j < 1024; j++)
				{
					temp[i][j] = MSG2138A_NEW_ITO_update_bin[i*1024+j];
				}
			}
			firmware_update_with_i(NULL, NULL, NULL, 0);
		}
	...
	if(buf != NULL && size != 0)
	{
		...
		else if(ENABLE_CTP_PS == val)
		{
			...
			if(msg2133a_data)
			{
				...
				input_sync(msg2133a_data->ps_input);
			}
		}
	}
...
#endif
				else if (Buf[5] == 0x00 || Buf[5] == 0xff)
				{
					Touch_State = Touch_State_Touch_up;
				}
				else
				{
...
static int msg2133_report_data(struct Data_CNT *device_contex)
{
	...
	if(msg2133_read_data()){
		...
		else if (Touch_State == Touch_State_Touch_up)
		{
			if(kave_VK==1)
			{
				if(key_id == 0x01)
...
static int msg2133a_power_on(bool on)
{
	int rc;
	...
	rc = regulator_enable(regu);//regulator_enable(device_contex->vdd);
	if (rc) {
		printk("Regulator vdd enable failed rc=%d\n", rc);
		return rc;
	}

	rc = regulator_enable(regu1);//regulator_enable(device_contex->vcc_i2c);
	if (rc) {
...
power_off:
	rc = regulator_disable(regu);
	if (rc) {
		dev_err("Regulator vdd disable failed rc=%d\n", rc);
		return rc;
	}

	rc = regulator_disable(regu1);
	if (rc) {
...
static int msg2133a_chip_init(void)//struct Data_CNT *device_contex,struct i2c_client *client)
{
	int err = 0;
	...
	pixcir_irq = gpio_to_irq(INT_GPIO);
	printk("msg2133 touch irq=%x\n",pixcir_irq);
	gpio_direction_output(RST_GPIO, 1);
	msleep(3);
	gpio_set_value(RST_GPIO, 0);
	msleep(10);
...
#ifdef CONFIG_PM
static int msg2133a_ts_suspend(struct Data_CNT *device_contex)//struct device *dev)
{
	//struct Data_CNT *device_contex = dev_get_drvdata(dev);
	int err;
	if (device_contex->suspended) {
		dev_info(&device_contex->client->dev, "Already in suspend state\n");
		return 0;
	}
	...
	err = msg2133a_power_on(false);
	mutex_unlock(&msg21xx_mutex);
	if (err) {
		dev_err(&device_contex->client->dev, "power off failed");
		goto pwr_off_fail;
	}
...
static int msg2133a_ts_resume(struct Data_CNT *device_contex)//struct device *dev)
{
	//struct Data_CNT *device_contex = dev_get_drvdata(dev);
	int err;

	mutex_lock(&msg21xx_mutex);
	err = msg2133a_power_on(true);
	if (err) {
		dev_err(&device_contex->client->dev, "power on failed");
		mutex_unlock(&msg21xx_mutex);
		return err;
	}
	if (gpio_is_valid(device_contex->rst)) {
...
#if defined(CONFIG_HAS_EARLYSUSPEND)//(for PR753200)
static void msg21xx_early_suspend(struct early_suspend *handler)
{
        struct Data_CNT *msg21_data = container_of(handler,
                                                   struct Data_CNT,
                                                   early_suspend);
        ...
        msg2133a_ts_suspend(msg21_data);
}
		...
		const struct i2c_device_id *id)
{
	...
	device_contex->irq=msg2133a_chip_init();//device_contex,client);
	device_contex->rst=RST_GPIO;
	printk("xcm %x ,%d\n",device_contex->rst,device_contex->irq);
	...
	input_dev->name = "ms-msg21xx";
#ifdef CONFIG_TOUCHSCREEN_MSG2133A_PSENSOR
	device_contex->ps_input->name = "p_sensor";
#endif
   ...
   //     input_set_abs_params(input_dev, ABS_MT_TRACKING_ID, 0, 2, 0, 0);
	...
	if(err < 0)
	{
		pr_err("%s:i2c  cannot read device type\n", __func__);
		goto exit_irq_request_failed;
	}

	printk("msg2133 0000\n");
	//device_contex->irq = INT_GPIO;//pdata->irq_gpio;	
...
#if defined(CONFIG_HAS_EARLYSUSPEND) //(for PR753200)  
		device_contex->early_suspend.level = EARLY_SUSPEND_LEVEL_STOP_DRAWING;
		device_contex->early_suspend.suspend = msg21xx_early_suspend;
		device_contex->early_suspend.resume = msg21xx_late_resume;
		register_early_suspend(&device_contex->early_suspend);
#endif
...
irq_free:
	free_irq(device_contex->irq, device_contex);
#endif
...
#ifdef CONFIG_TOUCHSCREEN_MSG2133A_PSENSOR
input_unregister_device(device_contex->ps_input);
device_contex->ps_input=NULL;
#else
...
exit_devmalloc_data_failed:	
	return -ENOMEM;
}
static int s3c_ts_remove(struct i2c_client *client)
{
#if defined(CONFIG_HAS_EARLYSUSPEND)//(for PR753200)
	struct Data_CNT *data = i2c_get_clientdata(this_client);
