...
static irqreturn_t lcdc_isr(int irq, void *data)
{
	...
	if(SPRD_OVERLAY_STATUS_STARTED == lcdc->overlay_state){
		...
		lcdc->vsync_done = 1;
#ifndef CONFIG_FB_LCD_TE_ESD_SUPPORT
		lcdc_clk_disable();
#endif
		if (dev->vsync_waiter) {
...
#ifdef CONFIG_FB_LCD_TE_ESD_SUPPORT
	if (val & 2) {
		dev->last_te = jiffies;
		lcdc_write(2, LCDC_IRQ_CLR);
		pr_debug(KERN_INFO "lcdc_te_isr !\n");
	}
#endif
	return IRQ_HANDLED;
}
...
static void lcdc_hw_init(void)
{
	...
	lcdc_set_bits((3<<0), LCDC_IRQ_CLR);
}
...
#ifdef CONFIG_FB_LCD_TE_ESD_SUPPORT
int sprd_lcdc_esd_reset(struct sprdfb_device *dev){
	sprd_lcdc_reset();
	lcdc_hw_init();
	sprd_lcdc_init(dev);
...
//    sprd_lcdc_refresh(dev);  
    dev->ctrl->refresh(dev);    //leihui add for refresh lcd to make sure TE signal could be checked really
	return 0;
}
#endif
...
static int32_t sprd_lcdc_refresh (struct sprdfb_device *dev)
{
	...
	if(!dev->ESD_work_start){
		...
		lcdc_set_bits(BIT(1),LCDC_IRQ_EN);
	}
#endif
