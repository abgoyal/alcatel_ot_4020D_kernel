...
extern int sprd_lcdc_esd_reset(struct sprdfb_device *dev);
#endif
...
static void ESD_work_func(struct work_struct *work)
{
	...
	if(time_after(jiffies, dev->last_te +3*HZ))
	{
		pr_debug("jiffies > last_te+3*HZ\n");
		pr_debug("panel reset\n");
		sprd_lcdc_esd_reset(dev);
	}
	else
	{
		pr_debug("dev->last_te=%llun",dev->last_te);
	}


	if(0 != dev->enable){
		//printk("sprdfb: reschedule ESD workqueue!\n");
...
#ifdef CONFIG_FB_LCD_TE_ESD_SUPPORT
	printk("sprdfb: Init ESD work queue!\n");
	INIT_DELAYED_WORK(&dev->ESD_work, ESD_work_func);
	dev->ESD_timeout_val=1000;
