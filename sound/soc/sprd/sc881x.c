...
EXPORT_SYMBOL(audio_switch_hook_register);
			    ...
			    struct snd_kcontrol *k, int event)
{
	...
	sc881x_dbg("Leaving %s\n", __func__);
        if (! !SND_SOC_DAPM_EVENT_ON(event) ) {
            if (audio_hook && audio_hook->earpiece) {
                audio_hook->earpiece();
            }

			g_spk_rec_ldo_pa |= 0x1;
			arch_audio_ldo_pa_enable();
        } else {
			g_spk_rec_ldo_pa &= 0xFE;
			arch_audio_ldo_pa_disable();
		}
	return 0;
}
			    ...
			    struct snd_kcontrol *k, int event)
{
	...
	if (! !SND_SOC_DAPM_EVENT_ON(event))
        ...
        if (! !SND_SOC_DAPM_EVENT_ON(event) ) {
            if (audio_hook && audio_hook->speaker) {
                audio_hook->speaker();
            }
			g_spk_rec_ldo_pa |= 0x2;
        } else {
			g_spk_rec_ldo_pa &= 0xFD;
		}
...
static int sc881x_late_probe(struct snd_soc_card *card)
{
	...
	sc881x_ext_control(&card->dapm);
        ...
        if (audio_hook && audio_hook->init) {
            if (audio_hook->init()) {
                sc881x_dbg("audio_switch_hook init fail\n");
                return -1;
            }
        }
#if 0
