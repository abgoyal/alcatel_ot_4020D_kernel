...
/* some SOC will move this into codec module */
static inline int arch_audio_ldo_pa_disable(void)
{
	int ret = 0;

#if FIXED_AUDIO
	unsigned int reg = sci_adi_read(ANA_AUDIO_PA_CTRL1) & 0xFFFF;
	if (g_spk_rec_ldo_pa & 0x2) {
		ret = sci_adi_raw_write(ANA_AUDIO_PA_CTRL1, reg & 0xFEFF);
	}
#endif
...
/* some SOC will move this into codec module */
static inline int arch_audio_ldo_pa_enable(void)
{
	int ret = 0;

#if FIXED_AUDIO
	unsigned int reg = sci_adi_read(ANA_AUDIO_PA_CTRL1) & 0xFFFF;
	ret = sci_adi_raw_write(ANA_AUDIO_PA_CTRL1, (reg | 0x100) & 0xFDFF);
#endif
...
static inline int arch_audio_inter_pa_disable(void)
{
	...
	if (ret < 0)
		return ret;
	if ( g_spk_rec_ldo_pa & 0x1 ) {
		ret = sci_adi_raw_write(ANA_AUDIO_PA_CTRL1, 0x1142);
		printk("g_spk_rec_ldo_pa = 0x%x\n",g_spk_rec_ldo_pa);
	} else {
