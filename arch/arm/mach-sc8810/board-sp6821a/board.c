...
static struct platform_device charge_led_device; 
...
static struct i2c_board_info i2c3_boardinfo[] = {
	{ 
		I2C_BOARD_INFO("REALTEK_FM",0x12),
	},
};
...
*/
static void audio_switch_speaker( void )
{
    /* switch to speaker */
    gpio_set_value(GPIO_REC_SWITCH, 0);
    return;
}

static void audio_switch_earpiece( void )
{

    /* switch to earpiece */
    gpio_set_value(GPIO_REC_SWITCH, 1);
    return;
}
...
static int audio_switch_init( void )
{
    ...
    ret = gpio_request(GPIO_REC_SWITCH, "GPIO_REC_SWITCH");
    if (ret) {
        pr_err("%s: request gpio error\n", __func__);
        return ret;
    }

    gpio_direction_output(GPIO_REC_SWITCH, 0);

    return 0;
}
static int sc8810_add_misc_devices(void)
{
        static struct audio_switch_hook audio_hook = {0};
        audio_hook.init = audio_switch_init;
        audio_hook.speaker = audio_switch_speaker;
        audio_hook.earpiece = audio_switch_earpiece;
		
		audio_switch_hook_register(&audio_hook);
