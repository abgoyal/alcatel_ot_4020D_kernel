...
//Realtek add
int hci_load_link_key_type(void __user *arg) {
		struct hci_dev *hdev;
		struct hci_conn *conn = NULL;
		__u8 __user *ptr = arg;
	
		struct hci_link_key_type_info lt;

		if (copy_from_user(&lt, ptr, sizeof(struct hci_link_key_type_info))) {
			BT_DBG("RTK: copy_from_user fail");
			return -EFAULT;
		}
	
		hdev = hci_dev_get(lt.dev_id);
		if (!hdev)
				return -ENODEV;
	
		conn = hci_conn_hash_lookup_ba(hdev, ACL_LINK, &lt.bdaddr);
		if (conn && conn->key_type == 0xFF) {
			conn->key_type = lt.type;
			printk("RTK: conn->key_type = %d", conn->key_type);
		}
